...
public class NotificacionInicioRutaRabbitMQListener {
  ...
  public void handleNotificacionInicioRuta(NotificacionInicioRutaEvent event) {
    ...
    String baseMensaje =
        String.format(
            "¡Tu donación está en camino! El chofer %s inició la ruta %s en el camión %s. Sigue tu entrega en vivo aquí: %s",
            event.getNombreChofer(),
            event.getRutaId(),
            event.getPatenteCamion(),
            event.getLinkSeguimiento());
  ...
  private void enviarNotificaciones(
      List<NotificacionInicioRutaEvent.ContactoInfo> contactos, String mensaje) {
    ...
    for (NotificacionInicioRutaEvent.ContactoInfo contacto : contactos) {
      try {
        notificadorService.enviarNotificacion(
            contacto.getDestinatario(), mensaje, contacto.getMedio());
        log.info(
            "Notificación enviada a {} vía {}", contacto.getDestinatario(), contacto.getMedio());
      } catch (Exception e) {
