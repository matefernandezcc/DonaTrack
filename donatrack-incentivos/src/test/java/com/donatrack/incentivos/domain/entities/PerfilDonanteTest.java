 ...
 *
 * <p>Cubren: - Estado inicial de un perfil nuevo (categoría COLABORADOR) - Registro de donaciones y
 * actualización de métricas - Progresión de misiones e insignias - Procesamiento de inactividad
 * (corte de racha a los 30 días sin donar) - Avance de categoría: COLABORADOR → SOSTENEDOR →
 * TRANSFORMADOR
 */
public class PerfilDonanteTest {
  ...
  public void testCompletarPrimeraMisionDeColaborador() {
    ...
    assertTrue(
        perfil.getInsigniasObtenidas().size() >= 1,
  ...
  public void testProcesarInactividadCortaRachaTras30Dias() {
    ...
    RegistroDonacion donacionVieja = crearRegistro(YearMonth.from(hace31Dias), hace31Dias, 1, true);
    ...
    assertNotNull(
        perfil.getMetricas().getFechaCorteRacha(),
        "Al superar los 30 días sin donar, la racha debe cortarse (fechaCorteRacha)");
    assertEquals(
        1,
        perfil.getMetricas().obtenerTodasLasDonaciones().size(),
  ...
  public void testProcesarInactividadNoCortaRachaDentroDe30Dias() {
    ...
    RegistroDonacion donacion = crearRegistro(YearMonth.from(hace29Dias), hace29Dias, 1, true);
  ...
  public void testProcesarInactividadNoCortaSiDonoHoy() {
    RegistroDonacion donacionHoy = crearRegistro(YearMonth.now(), LocalDate.now(), 1, true);
  ...
  public void testAvanceDeCategoriaProgresivo() {
    ...
    assertTrue(
        perfil.getInsigniasObtenidas().size() >= 2,
        "Debería haber ganado al menos 2 insignias con 6 donaciones ricas");
  }

  // ==================== Helpers ====================

  private RegistroDonacion crearRegistro(
      YearMonth mes, LocalDate fecha, int cantBienes, boolean exitosa) {
    return new RegistroDonacion(
        UUID.randomUUID(),
        cantBienes,
        Set.of("General"),
        exitosa ? UUID.randomUUID() : null,
        mes,
        fecha);
  }

  private RegistroDonacion crearRegistro(
      YearMonth mes, LocalDate fecha, int cantBienes, boolean exitosa, Set<String> categorias) {
    return new RegistroDonacion(
        UUID.randomUUID(), cantBienes, categorias, exitosa ? UUID.randomUUID() : null, mes, fecha);
  }
}
