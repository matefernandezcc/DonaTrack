...
public class MetricasDonanteTest {
  ...
  public void testRegistrarDonacionConIdDuplicadoActualiza() {
    ...
    RegistroDonacion donacion1 =
        new RegistroDonacion(idDonacion, 3, Set.of("Ropa"), UUID.randomUUID(), YearMonth.now());
    RegistroDonacion donacion2 =
        new RegistroDonacion(
            idDonacion, 5, Set.of("Ropa", "Alimentos"), UUID.randomUUID(), YearMonth.now());
  ...
  private RegistroDonacion crearDonacion(UUID id, boolean exitosa) {
    return new RegistroDonacion(
        id, 1, Set.of("General"), exitosa ? UUID.randomUUID() : null, YearMonth.now());
  }
}
