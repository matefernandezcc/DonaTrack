...
public class RegistrarActividadDonacionUseCase {
  ...
  public RegistrarActividadDonacionUseCase(
  ...
  public void ejecutar(UUID donanteId, ActividadDonacionDTO actividad) {
    PerfilDonante perfil =
        perfilDonanteRepository.findById(donanteId).orElse(new PerfilDonante(donanteId));

    int insigniasAntes = perfil.getInsigniasObtenidas().size();
    com.donatrack.incentivos.domain.entities.categoria.CategoriaDonante categoriaAntes =
        perfil.getCategoria();

    com.donatrack.incentivos.domain.entities.RegistroDonacion donacion =
        new com.donatrack.incentivos.domain.entities.RegistroDonacion(
            actividad.getIdDonacion(),
            actividad.getCantidadBienes(),
            actividad.getCategorias() != null
                ? new java.util.HashSet<>(actividad.getCategorias())
                : null,
    ...
    com.donatrack.incentivos.domain.entities.categoria.CategoriaDonante categoriaDespues =
        perfil.getCategoria();
    ...
    if (categoriaCambiada) {
      notificacionPort.enviarNotificacion(
          new NotificacionRequest(
              "donante" + donanteId + "@test.com",
              "¡Increíble! Has subido de categoría a " + categoriaDespues.name(),
              "EMAIL"));
    }
