...
public class GestorEventosLogistica {
  ...
  public void procesarInicioRuta(UUID idRuta, List<UUID> donacionesEnRuta) {
    for (UUID idDonacion : donacionesEnRuta) {
      ...
      String mensaje =
          "¡Tu donación está en camino! Seguila en vivo en la ruta: http://mapa.donatrack.com/ruta/"
              + idRuta;
      ...
      if (beneficiario != null) {
        ...
        personaBeneficiario.ifPresent(
            p -> {
              if (p.getContacto() != null) {
                notificadorPort.notificar(
                    p.getContacto(), mensaje, p.getContacto().getMedioPredeterminado());
              }
            });
      }

      // Notificar al donante original
      Optional<DonacionOriginal> originalOpt =
          donacionOriginalRepository.buscarPorIdDonacion(idDonacion);
      originalOpt.ifPresent(
          orig -> {
            Donante donante = orig.getDonante();
            if (donante != null) {
              Optional<Persona> personaDonante = personaRepository.buscarPorRolId(donante.getId());
              personaDonante.ifPresent(
                  p -> {
                    if (p.getContacto() != null) {
                      notificadorPort.notificar(
                          p.getContacto(), mensaje, p.getContacto().getMedioPredeterminado());
  ...
  public void procesarEntregaExitosa(UUID idDonacion, String comprobante) {
    ...
    donacion.cambiarEstado(
        EstadoDonacion.ENTREGADA, "Entrega exitosa. Comprobante: " + comprobante, null);
    donacionRepository.guardar(donacion);

    String mensaje =
        "Tu donación con ID " + idDonacion + " ha sido entregada con éxito a la entidad asignada.";
  ...
  public void procesarEntregaFallida(UUID idDonacion, String motivo) {
    ...
    String mensaje =
        "La entrega de la donación con ID " + idDonacion + " falló debido a: " + motivo;
