...
public class RecepcionDonacionesService implements RecepcionDonacionesUseCase {
  ...
  public DonacionOriginal recibir(CargaBienesRequestDTO requestDTO) {
    // 1. Obtener entidades
    Persona donantePersona =
        personaRepository
            .buscarPorId(requestDTO.idDonante())
            .orElseThrow(() -> new IllegalArgumentException("Donante no encontrado"));
    Persona adminPersona =
        personaRepository
            .buscarPorId(requestDTO.idAdministrador())
            .orElseThrow(() -> new IllegalArgumentException("Administrador no encontrado"));

    Donante donante =
        donantePersona.getRoles().stream()
    ...
    List<Bien> bienesBrutos =
        requestDTO.bienesBrutos().stream().map(this::mapearBien).collect(Collectors.toList());

    // 3. Crear acta de Recepción (Entidad)
    DonacionOriginal recepcion =
        new DonacionOriginal(
            "Carga de donación original", donante, requestDTO.idAdministrador().toString());
  ...
  private Bien mapearBien(BienDTO dto) {
    ...
    Subcategoria subcategoriaMock =
        new Subcategoria(dto.nombreSubcategoria(), "Subcategoría generada");
