...
public class ReplanificarEntregaUseCase {
  ...
  public ReplanificarEntregaUseCase(
      DonacionRepository donacionRepository, ApplicationEventPublisher eventPublisher) {
  ...
  public void replanificar(UUID idDonacion, String adminResponsableId) {
    ...
    if (donacion.getEstado() != EstadoDonacion.ENTREGA_FALLIDA) {
      throw new IllegalStateException(
          "Solo se puede replanificar una donación en estado ENTREGA_FALLIDA");
    }

    // Vuelve al estado ASIGNADA para que el sistema de ruteo pueda volver a considerarla
    donacion.cambiarEstado(
        EstadoDonacion.ASIGNADA, "Replanificación solicitada", adminResponsableId);
