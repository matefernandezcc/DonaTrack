...
public class ConfirmarRecepcionYNotificarUseCaseTest {
  ...
  void setUp() {
    ...
    useCase =
        new ConfirmarRecepcionYNotificarUseCase(
            donacionRepo, donacionOriginalRepo, personaRepo, eventPublisher);
  ...
  public void confirmaRecepcionYPublicaEvento() {
    ...
    Contacto contactoDonante =
        new Contacto("donante@test.com", "1111", "2222", MedioContacto.CORREO);
    PersonaHumana personaDonante =
        crearPersona("donante@test.com", contactoDonante, "Juan", "Pérez");
    ...
    EntregaRealizadaEvent entregaEvent =
        new EntregaRealizadaEvent(
    ...
    ArgumentCaptor<NotificacionEntregaExitosaEvent> captor =
        ArgumentCaptor.forClass(NotificacionEntregaExitosaEvent.class);
  ...
  public void donacionInexistenteNoHaceNada() {
    ...
    EntregaRealizadaEvent event =
        new EntregaRealizadaEvent(idInexistente, List.of(), "AA-123-BB", LocalDateTime.now());
  ...
  public void eventoContieneContactosDelDonanteYEntidad() {
    ...
    PersonaHumana personaDonante =
        crearPersona("donante@email.com", contactoDonante, "María", "López");
    when(personaRepo.buscarPorRolId(donante.getId())).thenReturn(Optional.of(personaDonante));

    Contacto contactoBenef = new Contacto(null, "1155550000", null, MedioContacto.TELEFONO);
    PersonaHumana personaBenef =
        crearPersona("entidad@email.com", contactoBenef, "Entidad", "Solidaria");
    when(personaRepo.buscarPorRolId(beneficiario.getId())).thenReturn(Optional.of(personaBenef));

    when(donacionRepo.buscarPorId(donacionId)).thenReturn(Optional.of(donacion));

    EntregaRealizadaEvent entregaEvent =
        new EntregaRealizadaEvent(donacionId, List.of(), "CC-789-DD", LocalDateTime.now());
  ...
  private PersonaHumana crearPersona(
      String email, Contacto contacto, String nombre, String apellido) {
