...
public class GestorEventosLogisticaTest {
  ...
  void setUp() {
    ...
    gestor =
        new GestorEventosLogistica(
            mockNotificador, mockDonacionRepo, mockPersonaRepo, mockDonOrigRepo);
  ...
  public void testProcesarInicioRutaNotificaBeneficiario() {
    ...
    when(mockDonacionRepo.buscarPorId(donacionId)).thenReturn(Optional.of(donacion));
    when(mockPersonaRepo.buscarPorRolId(beneficiario.getId()))
        .thenReturn(Optional.of(personaBenef));
